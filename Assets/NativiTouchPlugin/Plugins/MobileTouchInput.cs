...
using MobileTouchPlugin;

public class MobileTouchInput : MonoBehaviour { //メッセージを受け取るために
	...
	/*	 For Reference */
	public static MobileTouchInfo[] touches;

	/*	 For Calculate */
	private List<MobileTouchInfo> beforeTouches;
	private	List<MobileTouchInfo> currentTouches;
	...
	public void Initialization() {
		touches = new MobileTouchInfo[0];
		beforeTime = 0;
		currentTime = 0;
		deltaTime = 0.0f;
		beforeTouches = new List<MobileTouchInfo> ();
		currentTouches = new List<MobileTouchInfo> ();
	...
	public void onMessageCallback(String message) {
		List<MobileTouchInfo> targetTouches = new List<MobileTouchInfo> ();
		...
		for (int n = 0, size = int.Parse (values [1]); n < size;n++) {
			MobileTouchInfo touchInfo = new MobileTouchInfo ();
	...
	/// </summary>
	private void registerTouches(List<MobileTouchInfo> targetTouches) {
	...
	/// </summary>
	private void unregisterTouches(List<MobileTouchInfo> targetTouches, TouchPhase phase) {
	...
	/// Updates the touches.
	/// </summary>
	private void updateTouches(List<MobileTouchInfo> targetTouches) {
		foreach (var target in targetTouches) {
			for (int n = 0; n < currentTouches.Count; n++) {
	...
	void Update() {
		if (other) {
			...
			if (currentTouches.Count > 0) {
				beforeTouches = new List<MobileTouchInfo> (currentTouches);
	...
	/*
		void OnGUI() {
			...
			for (int i = 0; i < touches.Length; i++) {
				MobileTouchInfo info = touches [i];
